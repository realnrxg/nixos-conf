...
Item {
    ...
    readonly property string fastfetchLink: Quickshell.env("HOME") + "/.config/fastfetch/config.jsonc"
    ...
    function readCurrentFastfetchName() {
        ...
        const target = String(xhr.responseText || "").trim().split("/").pop() || "";
        return root.stripExt(target);
    }

    function currentFastfetchIndex() {
        const name = root.readCurrentFastfetchName();
        root.activeFastfetch = name;

        if (!name)
            return -1;

        for (let i = 0; i < wallModel.count; i += 1) {
            if (wallModel.get(i).baseName === name)
                return i;
        }
    ...
    function applyFastfetch(filePath) {
        ...
        let file = String(filePath).replace(/^file:\/\//, "");
        file = decodeURIComponent(file);
        const base = root.stripExt(file.split("/").pop());
        root.activeFastfetch = base;
        const esc = (str) => String(str).replace(/(["\\$`])/g, '\\$1');

        const script = `
        (
            CFG="${esc(root.fastfetchConfigsDir)}/${esc(base)}.jsonc"
            LINK="${esc(root.fastfetchLink)}"
            if [[ -f "$CFG" ]]; then
                ln -sf "$CFG" "$LINK"
            fi
        ) > /tmp/qs_fastfetch_apply.log 2>&1 & disown
        `;
    ...
    function loadNextBatch() {
        if (root.allLoaded || root.isLoadingBatch)
            return;
        ...
        for (let i = start; i < end; i += 1) {
            ...
            wallModel.append({
                fileName: fileName,
                baseName: root.stripExt(fileName),
    ...
    ColumnLayout {
        ...
        ListView {
            ...
            Keys.onReturnPressed: (event) => {
                ...
                if (entry)
                    root.applyFastfetch(entry.fileUrl);
                event.accepted = true;
            }
            ...
            delegate: Item {
                ...
                readonly property bool isActive: baseName === root.activeFastfetch
                ...
                MouseArea {
                    ...
                    onDoubleClicked: root.applyFastfetch(fileUrl)
                }
            }
        }
